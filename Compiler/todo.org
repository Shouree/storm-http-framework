...
* Improvements [6/25]
  - [ ] In the language server: Add the ability to tell the parser to abort parsing after some time.
    In cases where parsing takes much time, the results are typically very bad anyway, so it might be
    a good strategy to actually just give up if parsing takes more than 100 ms or so, and try again later
    (at least when re-parsing parts of the AST, maybe not on the initial parse, or in other situations).
