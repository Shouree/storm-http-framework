...
#endif
#define _WINSOCKAPI_ // To avoid warnings when including windows.h
...
void pem_locate(char* pem_str,
  ...
  char** end)
{
  // Note: Modified to fix compile errors.
  char c;
  const pem_type_desc* desc;

  *begin = NULL;
  *end = NULL;

  desc = &pem_sections[type];
  ...
  size_t      errmsg_len)
{
  ...
  {
    // Note: Moved here to avoid compilation errors.
    CERT_BLOB cert_blob;
    void* context = NULL;
...
typedef struct _WIN7_CERT_CHAIN_ENGINE_CONFIG {
  DWORD      cbSize;
  ...
  size_t          errmsg_len)
{
  ...
  if (hStore)
  {
    // Note: Modified to not lose support for hExclusiveRoot for older WinSDKs.
    WIN7_CERT_CHAIN_ENGINE_CONFIG EngineConfig = { 0 };
    EngineConfig.cbSize = sizeof(EngineConfig);
    EngineConfig.hExclusiveRoot = hStore;
    if (!CertCreateCertificateChainEngine((PCERT_CHAIN_ENGINE_CONFIG)&EngineConfig, &hChainEngine))
    {
