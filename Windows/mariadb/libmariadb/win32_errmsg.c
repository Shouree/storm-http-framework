...
// Wrapper for snprintf. Does not exist in VS2008.
int wrap_snprintf(char *out, size_t size, const char *format, ...)
{
  int count;
  va_list ap;

  va_start(ap, format);
  count = vsnprintf_s(out, size, _TRUNCATE, format, ap);
  va_end(ap);
...
void ma_format_win32_error(char* buf, size_t buflen, DWORD code, _Printf_format_string_ const char* fmt, ...)
{
  ...
  *cur = 0;
  ...
  // NOTE: Modified from original to avoid compiler error:
  {
  struct map_entry
  {
    DWORD code;
...
#define ENTRY(x, y) {x,#x, y}
  ...
  if (entry)
  {
    wrap_snprintf(cur, end - cur, "%s. Error 0x%08lX(%s)", entry->msg, code, entry->sym);
  }
  else
  {
    ...
    wrap_snprintf(cur, end - cur, ". Error %lu/0x%08lX", code, code);
  }
  end[-1] = 0;
  }
}
