...
**TODO** Brief introduction to the structure used in the tutorial. Ask user to save in directory `chess`.
...
**TODO** Maybe briefly note that `frame ChessVisualizer` is equivalent to `class ChessVisualizer
extends Frame`, but it allows specifying layout.

**TODO** Why is `create` important -- it should be called last to actually create the frame. It is only needed in frames.

**TODO** Why is `waitForClose` important? Otherwise `main` returns, and Storm exits.
...
In order to draw to the window we need to first create a subclass to the
...
**TODO**: Why do we want to create the SolidBrush as a member of the class? All resources are
  handled this way to avoid transferring resources to the GPU each frame.
...
**TODO** Again: why do we want to save the Text layouts.
...
**TODO** The code above has been simplified. Maybe the text above needs to be modified.
...
**TODO** The code above has been updated in the source files to make it simpler. Update the code and text above to reflect changes.
...
class ChessPainter extends Painter {
    ...
    Bool render(Size me, Graphics g) {      
		...
		for (Nat x = 0; x < 8; x++) {
			Str piece = order[x] + "b";
			g.draw(images.get("pb"), Rect(Point(squareSz.w * x.float, squareSz.h), squareSz));
			g.draw(images.get(piece), Rect(Point(squareSz.w * x.float, squareSz.h * 0), squareSz));
		}

		for (Nat x = 0; x < 8; x++) {
			Str piece = order[x] + "w";
			g.draw(images.get("pw"), Rect(Point(squareSz.w * x.float, squareSz.h * 6), squareSz));
			g.draw(images.get(piece), Rect(Point(squareSz.w * x.float, squareSz.h * 7), squareSz));
		}
...
**TODO** The listing above is quite large, and does not contain the final code. Only show the
  relevant parts of the code above, and put the complete listing (including the About dialog) at the
  end of the page.
